...
const { t, i18n } = useTranslation();
  ...
  const isFormValid =
    ...
    const handleSignUp = async () => {
      ...
      setError('');
      try {
        const fullPhone = `${selectedCountry.code.replace('+', '')}${phoneNumber}`;
        const body = {
          phone: fullPhone,
          password,
        ...
        // Step 2: Send OTP immediately after successful registration
        try {
          await networkClient.post(API_ENDPOINTS.GET_OTP_SIGN_UP, { phone: fullPhone });
          console.log('OTP sent successfully');
        } catch (otpErr) {
          console.log('OTP send error (non-critical):', otpErr);
        ...
        Toast.show({ 
          type: 'success', 
          text1: 'Success', 
          text2: 'Account created! Please check your phone for OTP.' 
        });
